...
def data_load(args):
    ...
    nw = min([os.cpu_count(), batch_size if batch_size > 1 else 0, 8])  # number of
...
if __name__ == '__main__':
    ...
    start = time.time()
    if not args.with_ray:
        tr_loss, tr_acc, va_loss, va_acc,tr_time,va_time = train_validate_network(args)
        end = time.time()
        ...
        np.save('tr_time0.npy',tr_time)
        np.save('va_time0.npy',va_time)
